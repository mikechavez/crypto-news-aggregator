...
from src.crypto_news_aggregator.services.alert_notification_service import (
...
class MockEmailService:
    ...
    async def send_price_alert(self, *args, **kwargs):
        self.sent_alerts.append({"args": args, "kwargs": kwargs})
        return True
...
async def test_alert_notification(alert: AlertInDB):
    ...
    with (
        patch(
            "src.crypto_news_aggregator.services.alert_notification_service.email_service",
            mock_email_service,
        ),
        patch.object(
            alert_service, "_get_relevant_news", return_value=[]
        ),  # Mock _get_relevant_news to return empty list
    ):
        # Set initial price and last_triggered_price for the alert
        initial_price = 50000.0

        # Update the alert in the database with the initial price
        from src.crypto_news_aggregator.services.alert_service import (
        ...
        logger.info(
            f"Initial alert state - last_triggered_price: {alert.last_triggered_price}"
        )
        ...
        if alert.last_triggered_price != initial_price:
            logger.error(
                f"Alert update failed. Expected last_triggered_price={initial_price}, got {alert.last_triggered_price}"
            )
        ...
        logger.info(
            f"Verified alert update - last_triggered_price: {alert.last_triggered_price}"
        )
        ...
        with patch(
            "src.crypto_news_aggregator.services.alert_notification_service.price_service.get_bitcoin_price"
        ) as mock_get_price:
            ...
            logger.info(
                "✅ Test passed: Alert correctly not triggered for small price change"
            )
        ...
        # Update the alert
        updated_alert = await alert_svc.update_alert(
            alert_id=str(alert.id), user_id=alert.user_id, alert_update=update_data
        )

        if updated_alert is None:
            logger.error(
                "Failed to update alert in the database for large price change test"
            )
            raise AssertionError("Failed to update alert for large price change test")

        logger.info(
            f"Alert updated successfully for large price change test: {updated_alert}"
        )
        ...
        if alert.last_triggered_price != new_baseline_price:
            logger.error(
                f"Alert update failed. Expected last_triggered_price={new_baseline_price}, got {alert.last_triggered_price}"
            )
            raise AssertionError(
                f"Alert update failed. Expected last_triggered_price={new_baseline_price}, got {alert.last_triggered_price}"
            )
        ...
        logger.debug(
            f"Full alert object before large price change test: {alert.model_dump()}"
        )
        ...
        updated_alert = await alert_service.get_alert(str(alert.id))
        assert (
            updated_alert is not None
        ), "Test alert should still exist after processing"
        ...
        assert (
            "2.00%" in test_alert_emails[0]["html"]
        ), "Email should contain the price change"

        # Verify the alert was updated with the last triggered price
        assert (
            updated_alert.last_triggered_price == 51000.0
        ), "Alert should be updated with last triggered price"
