...
async def send_request(client: httpx.AsyncClient, payload: dict):
    ...
    try:
        if payload.get("stream", False):
            ...
            async with client.stream("POST", f"{BASE_URL}{ENDPOINT}", json=payload, timeout=30) as response:
                response.raise_for_status()
                async for chunk in response.aiter_bytes():
                    if chunk.strip():
                        print(chunk.decode('utf-8'))
        else:
            response = await client.post(f"{BASE_URL}{ENDPOINT}", json=payload, timeout=30)
            response.raise_for_status()
...
async def main():
    ...
    async with httpx.AsyncClient() as client:
        ...
        price_payload = {
            ...
            "messages": [{"role": "user", "content": "What is the current price of BTC and ETH?"}],
            "stream": False
        }
        ...
        sentiment_payload = {
            "model": "crypto-insight-agent",
            "messages": [{"role": "user", "content": "What is the market sentiment for Solana?"}],
            "stream": False
        }
        ...
        correlation_payload = {
            "model": "crypto-insight-agent",
            "messages": [{"role": "user", "content": "How does ETH correlate with BTC?"}],
            "stream": False
        }
