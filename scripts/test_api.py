...
def wait_for_task_completion(
    task_id: str, max_retries: int = 10, delay: float = 0.5
) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(
                f"Task {task_id} status: {status} (attempt {attempt + 1}/{max_retries})"
            )
            time.sleep(delay)
...
def run_tests():
    ...
    if not test_sentiment_analysis():
        logger.warning(
            "Sentiment analysis test had issues (may be expected if not fully implemented)"
        )
