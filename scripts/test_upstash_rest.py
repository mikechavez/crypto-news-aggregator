...
def test_upstash_rest():
    ...
    response = requests.post(f"{url}/set/{test_key}/{test_value}", headers=headers)

    print(f"SET Response: {response.status_code} - {response.text}")

    # Test GET command
    response = requests.get(f"{url}/get/{test_key}", headers=headers)

    print(f"GET Response: {response.status_code} - {response.text}")

    # Clean up
    response = requests.post(f"{url}/del/{test_key}", headers=headers)
