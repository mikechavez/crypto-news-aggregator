...
class APILoadTester:
    ...
    async def make_request(self, endpoint: str, method: str = "GET", params: Dict = None) -> Dict[str, Any]:
    ...
    async def test_concurrent_requests(self, endpoint: str, num_requests: int = 10) -> Dict[str, Any]:
    ...
    async def test_bitcoin_endpoints(self, num_requests: int = 5) -> Dict[str, Any]:
        ...
        for endpoint in endpoints:
            try:
                ...
                logger.info(f"Endpoint {endpoint}: {result['success_rate']:.1f}% success rate, {result['avg_duration_ms']:.1f}ms avg")
    ...
    async def test_ethereum_endpoints(self, num_requests: int = 5) -> Dict[str, Any]:
        ...
        endpoints = [
            "/api/v1/price/analysis/ethereum"
        ]
...
async def main():
    ...
    parser.add_argument("--requests", type=int, default=5, help="Number of concurrent requests per endpoint")
    parser.add_argument("--endpoints", choices=["bitcoin", "ethereum", "both"], default="both", help="Which endpoints to test")
        ...
        if args.endpoints in ["bitcoin", "both"]:
            ...
            for endpoint, result in bitcoin_results.items():
                if "error" not in result:
                    ...
                    print(f"  Min/Max Response Time: {result['min_duration_ms']:.1f}ms / {result['max_duration_ms']:.1f}ms")
