...
def setup_env():
    ...
    if env_path.exists():
        ...
        response = input("\nDo you want to update the existing .env file? (y/n): ").strip().lower()
        if response != 'y':
    ...
    print("Enter your Upstash Redis credentials (press Enter to keep existing values):\n")
    
    # Get or update Redis URL with working defaults
    current_redis_url = os.getenv('UPSTASH_REDIS_REST_URL', 'https://grown-oarfish-27991.upstash.io')
    redis_url = input(f"\n=== Redis Configuration ===\nUpstash Redis REST URL [{current_redis_url}]: ").strip()
    redis_url = redis_url if redis_url else current_redis_url
    
    # Get or update Redis token with working defaults
    current_redis_token = os.getenv('UPSTASH_REDIS_TOKEN', 'AW1XAAIjcDExYTU4YWM2OGRkNzg0OThhYTkyM2YxNDhkOGM1ZmM2OXAxMA')
    ...
    mongodb_uri = input(f"MongoDB Connection URI (e.g., mongodb+srv://<username>:<password>@<cluster>.mongodb.net/) [{current_mongodb_uri}]: ").strip()
