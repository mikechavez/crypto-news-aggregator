    ...
    async for db in get_session():
        try:
            ...
            if not user:
                ...
                user_in_create = UserCreateSQL(
                    email=user_email, username="testuser", password="password"
                )
            ...
            logger.info(
                f"Successfully created a test alert for {user.email} on {alert_in.crypto_id}."
            )
