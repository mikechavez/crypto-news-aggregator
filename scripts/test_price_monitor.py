...
async def setup_test_alerts() -> bool:
    ...
    try:
        ...
        print(f"[setup_test_alerts] Running in task: {current_task.get_name() if current_task else 'None'}")
    ...
    try:
        ...
        print(f"[setup_test_alerts] Deleted {delete_result.deleted_count} existing test alerts")
        ...
        result = await run_mongodb_operation(
            _insert_alerts,
            "insert_test_alerts"
        )
...
async def run_mongodb_operation(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            loop = asyncio.get_event_loop()
            print(f"[MongoDB] Running operation '{operation_name}' on loop {id(loop)} (attempt {attempt}/{max_retries})")
            ...
            if attempt == max_retries:
                print(f"[MongoDB] Operation '{operation_name}' failed after {max_retries} attempts")
                raise
...
async def cleanup():
    ...
    if hasattr(mongo_manager, '_async_client') and mongo_manager._async_client is not None:
