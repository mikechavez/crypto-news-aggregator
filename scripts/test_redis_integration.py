...
def print_env_vars():
    ...
    logger.info(f"UPSTASH_REDIS_REST_URL: {'Set' if settings.UPSTASH_REDIS_REST_URL else 'Not set'}")
    logger.info(f"UPSTASH_REDIS_TOKEN: {'Set' if settings.UPSTASH_REDIS_TOKEN else 'Not set'}")
    
    # Mask the token for security
    if settings.UPSTASH_REDIS_TOKEN:
        masked_token = f"{settings.UPSTASH_REDIS_TOKEN[:10]}...{settings.UPSTASH_REDIS_TOKEN[-4:]}"
...
def test_redis_integration():
    ...
    try:
        ...
        if retrieved == test_value:
            ...
            logger.error(f"❌ Get operation failed. Expected '{test_value}', got '{retrieved}'")
            return False
        ...
        if deleted == 1:
            ...
            logger.warning(f"⚠️  Delete operation may have failed. Expected to delete 1 key, got {deleted}")
        ...
        if should_be_none is None:
            ...
            logger.error(f"❌ Verification failed: Key still exists with value: {should_be_none}")
            return False
