...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("news_collection_test.log")],
)
...
async def test_news_collection():
    ...
    try:
        ...
        logger.info(
            f"✅ Successfully connected to NewsAPI. Found {len(sources.get('sources', []))} sources"
        )
    except Exception as e:
    ...
    try:
        ...
        logger.info(
            f"✅ Successfully connected to NewsAPI. Found {len(sources_response['sources'])} sources"
        )
        ...
        top_sources = [
            ...
            if "crypto" in s["name"].lower() or "coin" in s["name"].lower()
        ]
        logger.info(
            f"Found {len(top_sources)} crypto-related sources: {', '.join(top_sources[:5])}{'...' if len(top_sources) > 5 else ''}"
        )
        ...
        if top_sources:
            ...
            logger.info(
                f"Found {articles['totalResults']} total articles, showing {len(articles['articles'])}"
            )
    ...
    try:
        ...
        for source in test_sources:
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Available sources: {', '.join(source_names[:10])}{'...' if len(source_names) > 10 else ''}"
                    )
                except Exception as e:
                ...
                if new_articles > 0:
                    logger.info(
                        f"✅ Successfully collected {new_articles} new articles from {source}"
                    )
                    ...
                    try:
                        async with session_maker() as session:
                            ...
                            if latest_article:
                                ...
                                logger.info(
                                    f"  Published: {latest_article.published_at}"
                                )
                                logger.info(f"  URL: {latest_article.url}")
                    except Exception as e:
                        logger.warning(
                            f"Could not verify articles in database: {str(e)}"
                        )
                else:
                ...
                logger.error(
                    f"❌ Error collecting from source {source}: {str(e)}", exc_info=True
                )
                continue
        ...
        try:
            ...
            if total_new_articles > 0:
                logger.info(
                    f"✅ Successfully collected {total_new_articles} new articles from all sources"
                )
            ...
            logger.error(
                f"❌ Error collecting from all sources: {str(e)}", exc_info=True
            )
