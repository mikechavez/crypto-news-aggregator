...
class PriceMonitor:
    ...
    async def check_prices(self) -> None:
        ...
        try:
            ...
            context_articles = []
            if (
                abs(price_change_24h) >= 1.0
            ):  # Only fetch articles for significant movements
    ...
    async def _get_price_context(self, price_change: float) -> List[Dict[str, Any]]:
        ...
        try:
            # Get recent articles
            recent_articles = await article_service.get_recent_articles(
                hours=self.article_lookback_hours, limit=20
            )
