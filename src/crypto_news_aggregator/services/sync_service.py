...
class SyncService:
    ...
    def __init__(self, batch_size: int = 100):
        ...
        self.last_sync_time = datetime.now(timezone.utc) - timedelta(
            minutes=5
        )  # Initial sync window

    async def _convert_pg_to_mongo_article(
        self, pg_article: PGArticle
    ) -> Dict[str, Any]:
        """Convert a PostgreSQL article to MongoDB format."""
        source = (
            await pg_article.awaitable_attrs.source
            if hasattr(pg_article, "awaitable_attrs")
            else pg_article.source
        )
        ...
        return {
            ...
            "sentiment": (
                ...
                if pg_article.sentiment_score is not None
                else None
            ),
            "metadata": {
            ...
            for i in range(0, len(articles), self.batch_size):
                ...
                for pg_article in batch:
                    try:
                        ...
                        article_data = await self._convert_pg_to_mongo_article(
                            pg_article
                        )
                        ...
                        if synced_count % 10 == 0:
                            logger.info(
                                f"Synced {synced_count}/{len(articles)} articles"
                            )

                    except Exception as e:
                        logger.error(
                            f"Error syncing article {pg_article.id}: {str(e)}",
                            exc_info=True,
                        )
                        continue
    ...
    async def sync_sources_to_mongodb(self):
        ...
        try:
            ...
            for pg_source in sources:
                try:
                    ...
                    source_data = {
                        ...
                        "created_at": pg_source.created_at
                        or datetime.now(timezone.utc),
                        "updated_at": pg_source.updated_at
                        or datetime.now(timezone.utc),
                    }
                    ...
                    logger.error(
                        f"Error syncing source {pg_source.id}: {str(e)}", exc_info=True
                    )
                    continue
            ...
            logger.error(
                f"Error during source synchronization: {str(e)}", exc_info=True
            )
            raise
        finally:
            await session.close()
    ...
    async def sync_all(self):
        ...
        try:
            ...
            logger.info(
                f"Synchronization complete. Synced {sources_synced} sources and {articles_synced} articles."
            )
            return {
