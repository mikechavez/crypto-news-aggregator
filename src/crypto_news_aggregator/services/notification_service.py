...
class NotificationService:
    ...
    async def process_price_alert(
        ...
        """
        logger.info(
            f"Processing price alerts for {crypto_name} ({crypto_symbol}): ${current_price:.2f} ({price_change_24h:+.2f}%)"
        )

        # Get all active alerts for this cryptocurrency
        alerts = await get_active_alerts(db)
        alerts = [
            alert for alert in alerts if alert.symbol.upper() == crypto_symbol.upper()
        ]
            ...
            try:
                ...
                if self._should_trigger_alert(alert, price_change_24h):
                    ...
                    try:
                        ...
                        articles_to_include = (
                            context_articles if abs(price_change_24h) >= 1.0 else []
                        )
    ...
    def _should_trigger_alert(self, alert: Alert, price_change_24h: float) -> bool:
        ...
        if alert.last_triggered:
            ...
            if time_since_last < timedelta(minutes=60):
                logger.debug(
                    f"Alert {alert.id} is in cooldown. Last triggered {time_since_last.total_seconds() / 60:.2f} minutes ago."
                )
    ...
    async def _send_alert_notification(
        ...
        logger.info(
            f"Sent price alert email to {alert.user.email} for {crypto_symbol} {change_text}"
        )
