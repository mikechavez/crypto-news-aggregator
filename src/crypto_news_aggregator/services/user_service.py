...
class UserService:
    ...
    async def _get_collection(self):
        ...
        if self._db is None:
            ...
            logging.warning(
                f"[UserService._get_collection] Acquired async DB: type={type(self._db)}, value={self._db}"
            )
        collection = self._db[self.collection_name]
        collection = self.db[self.collection_name]
        logging.warning(
            f"[UserService._get_collection] collection type: {type(collection)}, value: {collection}"
        )
        return collection
    ...
    async def update_user(
        self, user_id: str, update_data: UserUpdate, updated_by_admin: bool = False
    ) -> Optional[UserInDB]:
    ...
    async def verify_email(self, token: str) -> bool:
        ...
        token_age = datetime.utcnow() - user.get(
            "email_verification_sent_at", datetime.utcnow()
        )
    ...
    async def update_subscription_preferences(
        self, user_id: str, preferences: UserSubscriptionPreferences
    ) -> Optional[UserInDB]:
    ...
    async def unsubscribe_user(
        self, token: str, email_type: Optional[str] = None
    ) -> bool:
