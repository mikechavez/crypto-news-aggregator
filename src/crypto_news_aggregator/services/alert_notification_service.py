...
class AlertNotificationService:
    ...
    async def _check_alert_condition(
        ...
        logger.info(f"[DEBUG] Alert details: condition={alert.condition}, threshold={alert.threshold}%, threshold_percent={alert.threshold_percent}%")
        logger.info(f"[DEBUG] Price data: last_triggered_price={alert.last_triggered_price}, current_price={current_price}")
        
        if not alert.last_triggered_price:
            # First time checking this alert, set baseline price
            logger.info(f"[DEBUG] No last_triggered_price for alert {alert.id}, setting baseline")
            return False, 0.0
        ...
        logger.info(f"[DEBUG] Change calculation: {current_price} - {alert.last_triggered_price} = {change_amount} ({change_percent:.2f}%)")
        
        # Check if the change meets the threshold based on alert condition
        threshold = alert.threshold_percent
        logger.info(f"[DEBUG] Checking condition: {alert.condition}, change={change_percent:.2f}% vs threshold={threshold}%")
        ...
        if alert.condition == "percent_up" and change_percent >= threshold:
            should_trigger = True
            logger.info(f"[DEBUG] Condition met: Price increased by {change_percent:.2f}% >= {threshold}%")
        elif alert.condition == "percent_down" and change_percent <= -threshold:
            should_trigger = True
            logger.info(f"[DEBUG] Condition met: Price decreased by {abs(change_percent):.2f}% >= {threshold}%")
        elif alert.condition == "above" and current_price >= alert.threshold:
            should_trigger = True
            logger.info(f"[DEBUG] Condition met: Price {current_price} >= {alert.threshold}")
        elif alert.condition == "below" and current_price <= alert.threshold:
            should_trigger = True
            logger.info(f"[DEBUG] Condition met: Price {current_price} <= {alert.threshold}")
        
        if should_trigger:
            logger.info(f"[ALERT] Alert {alert.id} TRIGGERED: {alert.condition} condition met with {change_percent:.2f}% change")
            return True, change_percent
            
        logger.info(f"[DEBUG] Alert {alert.id} NOT TRIGGERED: Condition not met")
        return False, change_percent
    
    async def _get_relevant_news(self, price_change_percent: float, limit: int = 3) -> List[Dict[str, Any]]:
        """
