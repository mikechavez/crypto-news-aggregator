...
class NewsCorrelator:
    ...
    def __init__(self):
        self.bitcoin_keywords = [
            ...
            'blockchain', 'satoshi', 'halving', 'mining', 'hash rate',
            'digital gold', 'store of value', 'crypto market'
        ]
        self.price_keywords = [
            'price', 'surge', 'plunge', 'rally', 'crash', 'drop', 'rise',
            'fall', 'soar', 'tumble', 'decline', 'increase', 'decrease',
            'pump', 'dump', 'bull', 'bear', 'bullish', 'bearish', 'ATH',
            'all-time high', 'support', 'resistance', 'breakout', 'break down'
        ]
    ...
    async def _get_collection(self):
        ...
        if not hasattr(self, '_collection'):
            self._collection = await mongo_manager.get_async_collection(COLLECTION_ARTICLES)
    ...
    def _calculate_article_relevance(self, article: ArticleInDB) -> float:
        ...
        keyword_score = sum(
            1 for keyword in self.bitcoin_keywords 
            if keyword in text
        ) / len(self.bitcoin_keywords)
        
        # Check for price-related keywords
        price_score = sum(
            1 for keyword in self.price_keywords 
            if keyword in text
                ...
                try:
                    ...
                    if relevance >= min_relevance:
                        scored_articles.append({
                            ...
                            'snippet': (article.description or '')[:200] + '...' if article.description else ''
                        })
                except Exception as e:
