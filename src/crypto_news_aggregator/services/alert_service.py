...
class AlertService:
    ...
    async def create_alert(
        self, alert: Union[AlertCreate, Dict[str, Any]]
    ) -> AlertInDB:
        """
        Create a new alert.
    ...
    async def update_alert(
        self, alert_id: str, update_data: Union[AlertUpdate, Dict[str, Any]], **kwargs
    ) -> Optional[AlertInDB]:
        """
        Update an alert.
        ...
        result = await collection.update_one({"_id": alert_oid}, {"$set": update_dict})
    ...
    async def update_alert(
        self, alert_id: str, user_id: str, alert_update: AlertUpdate
    ) -> Optional[AlertInDB]:
        """
        Update an alert.
        ...
        result = await collection.update_one(
            {"_id": ObjectId(alert_id), "user_id": user_id}, {"$set": update_data}
        )
    ...
    async def get_active_alerts_for_crypto(self, crypto_id: str) -> List[AlertInDB]:
        """
        ...
        """
        collection = await self._get_collection()
        query = {"crypto_id": crypto_id.lower(), "is_active": True}
    ...
    async def mark_alert_triggered(self, alert_id: str) -> None:
        ...
        await collection.update_one(
            {"_id": ObjectId(alert_id)}, {"$set": {"last_triggered": datetime.utcnow()}}
        )
