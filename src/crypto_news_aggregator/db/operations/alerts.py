            ...
            and_(
                ...
                or_(
                    ...
                    and_(
                        ...
                        AlertDB.threshold_percentage <= ((current_price / AlertDB.last_triggered_price - 1) * 100)
                        if AlertDB.last_triggered_price is not None
                        else True,
                        AlertDB.threshold_percentage <= ((current_price / AlertDB.created_at_price - 1) * 100)
                        if AlertDB.created_at_price is not None
                        else True
                    ),
                    # Price went down and alert is for price going down
                    and_(
                        AlertDB.direction.in_([AlertDirection.DOWN, AlertDirection.BOTH]),
                        AlertDB.threshold_percentage <= ((AlertDB.last_triggered_price / current_price - 1) * 100)
                        if AlertDB.last_triggered_price is not None
                        else True,
                        AlertDB.threshold_percentage <= ((AlertDB.created_at_price / current_price - 1) * 100)
                        if AlertDB.created_at_price is not None
                        else True
                    )
                )
