...
async def get_api_key(api_key_header: str = Security(api_key_header)) -> str:
    ...
    logger.debug(f"Validating API key: {api_key_header[:3]}...{api_key_header[-3:] if len(api_key_header) > 6 else '***'}")
    
    if api_key_header not in valid_api_keys:
        logger.warning(f"Invalid API key provided. Valid keys: {[k[:3] + '...' + k[-3:] if len(k) > 6 else '***' for k in valid_api_keys]}")
        raise HTTPException(
