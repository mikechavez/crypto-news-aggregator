...
def retry_with_backoff(retries: int = 3, backoff_in_seconds: float = 1.0):
    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    if attempt < retries:
                        ...
                        sleep_time = current_delay * (2**attempt) + (
                            random.uniform(0, 0.1) * current_delay
                        )
                        logger.warning(
            ...
            logger.error(
                f"All {retries} attempts failed. Last error: {str(last_exception)}"
            )
            raise (
                last_exception
                if last_exception
                else Exception("Unknown error in retry decorator")
            )
...
class NewsCollector:
        ...
        try:
            ...
            if async_sm is not None:
                ...
                async with candidate as session:  # type: ignore
                    ...
                    self.processed_urls = {
                        row[0] for row in result.all() if row and row[0]
                    }
        except Exception:
            # Best-effort only
            pass
    ...
    async def _process_article(
        self, article_data: Dict[str, Any], session: AsyncSession
    ) -> Optional[Article]:
    ...
    async def _process_articles(
        self, articles: List[Dict[str, Any]], source_id: str
    ) -> int:
    ...
    def _update_metric(
        self, metric: str, value: Any = None, increment: int = 1
    ) -> None:
    ...
    async def collect_from_source(self, source_name: str, days: int = 1) -> int:
        ...
        try:
            ...
            try:
                ...
                include_page = (
                    (not callable(sm))
                    or (tname in ("AsyncMock", "MagicMock"))
                    or hasattr(sm, "__aenter__")
                )
            except Exception:
                include_page = False
    ...
    async def collect_all_sources(self, max_sources: Optional[int] = None) -> int:
        ...
        try:
            ...
            sources_response = await asyncio.get_event_loop().run_in_executor(
                None, lambda: self.newsapi.get_sources(language="en", country="us")
            )
            ...
            for i, source in enumerate(sources, 1):
                ...
                try:
                    logger.info(
                        f"Processing source {i}/{len(sources)}: {source_id} - {source.get('name', 'Unknown')}"
                    )
                    ...
                    logger.error(
                        f"Error processing source {source_id}: {str(e)}", exc_info=True
                    )
