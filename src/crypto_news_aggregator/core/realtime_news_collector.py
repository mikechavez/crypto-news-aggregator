...
class RealtimeNewsCollector:
    ...
    def __init__(self, newsapi_base_url: Optional[str] = None):
        ...
        self.base_url = (
            newsapi_base_url or settings.REALTIME_NEWSAPI_URL or "http://localhost:3000"
        )
    ...
    async def _load_processed_urls(self):
        ...
        async with self.session_maker() as session:
            ...
            logger.info(
                f"Loaded {len(self.processed_urls)} processed URLs from database"
            )
    ...
    async def collect_from_source(self, source_id: str) -> int:
        ...
        from_date = (datetime.now(timezone.utc) - timedelta(days=3)).strftime(
            "%Y-%m-%d"
        )
    ...
    async def _process_articles(
        ...
        async with self.session_maker() as session:
            ...
            try:
                ...
                logger.info(
                    f"Processed {len(articles)} articles, {new_articles} were new"
                )
            ...
            try:
                ...
                for source in sources:
                    try:
                        ...
                        logger.error(
                            f"Error processing source {source['id']}: {str(e)}"
                        )
