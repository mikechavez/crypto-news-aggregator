                ...
                try:
                    ...
                    if e.response.status_code == 429:  # Rate limited
                        ...
                        if retries > max_retries:
                            raise APIError(f"Max retries ({max_retries}) exceeded for server error: {str(e)}", 
                        ...
                        logger.warning(f"Server error {e.response.status_code} (attempt {retries}/{max_retries}). Retrying in {sleep_time:.1f}s...")
                        await asyncio.sleep(sleep_time)
                        continue
                    # For other HTTP errors, raise immediately
                    raise APIError(f"API request failed: {str(e)}", status_code=e.response.status_code)
                    ...
                    jitter = backoff * 0.1  # Add ±10% jitter
                    sleep_time = backoff + (random.random() * 2 - 1) * jitter
                    logger.warning(f"Request failed (attempt {retries}/{max_retries}). Retrying in {sleep_time:.1f}s...")
                    await asyncio.sleep(sleep_time)
        finally:
        ...
        try:
            ...
            try:
                published_at = datetime.fromtimestamp(
                    raw_article.get('published_at', 0),
                    tz=timezone.utc
                )
            ...
            if raw_article.get('category'):
                categories.append(self._sanitize_html(raw_article['category'].get('name', '')))
