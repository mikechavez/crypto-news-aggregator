...
class CoinDeskSource(NewsSource):
        ...
        try:
            ...
            while count < limit:
                ...
                if rate_limit_remaining == 0:
                    wait_time = max(rate_limit_reset - time.time(), 0) + 1  # Add 1s buffer
                    if wait_time > 0:
                ...
                while retries <= max_retries:
                    try:
                        ...
                        rate_limit_remaining = int(rate_limit_remaining_str) if rate_limit_remaining_str and rate_limit_remaining_str.lower() != 'inf' else float('inf')
                        ...
                        logger.debug(f"Processed {articles_processed} articles from page {page}")
    ...
    def _extract_cryptocurrencies(self, text: str) -> List[str]:
        ...
        crypto_terms = [
            ...
            'polkadot', 'solana', 'dogecoin', 'shiba inu', 'avalanche',
            'chainlink', 'bitcoin cash', 'uniswap', 'cosmos', 'algorand',
            'vechain', 'filecoin', 'tezos', 'monero', 'stellar'
        ]
    ...
    def format_article(self, raw_article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(raw_article, dict):
            raise TypeError(f"Expected dict for raw_article, got {type(raw_article).__name__}")
        ...
        try:
            ...
            missing_fields = [field for field in required_fields if field not in raw_article]
            if missing_fields:
            ...
            try:
                ...
                logger.warning(f"Invalid publication date '{raw_article['published_at']}': {str(e)}")
            ...
            if article_url:
                if not article_url.startswith(('http://', 'https://')):
                    article_url = f"{self.base_url.rstrip('/')}/{article_url.lstrip('/')}"
            ...
            description = self._sanitize_html(str(raw_article.get('description', ''))).strip()
            ...
            image_url = None
            if isinstance(raw_article.get('image'), dict) and 'original_url' in raw_article['image']:
                image_url = str(raw_article['image']['original_url']).strip()
            
            # Extract author information with validation
            author_name = ""
            if isinstance(raw_article.get('author'), dict) and 'name' in raw_article['author']:
                author_name = self._sanitize_html(str(raw_article['author']['name'])).strip()
            ...
            formatted = {
                ...
                'tags': tags,
                'cryptocurrencies': sorted(list(set(cryptocurrencies))),  # Remove duplicates
            ...
            error_msg = f"Error formatting article {raw_article.get('id', 'unknown')}: {str(e)}"
