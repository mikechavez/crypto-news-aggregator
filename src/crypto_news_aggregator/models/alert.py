...
class AlertBase(BaseModel):
    ...
    user_id: PyObjectId = Field(
        ..., description="ID of the user who created the alert (ObjectId string)"
    )
    ...
    threshold: float = Field(
        ..., description="Price or percentage threshold for the alert"
    )
    is_active: bool = Field(True, description="Whether the alert is active")
    cooldown_minutes: int = Field(
        60,
        description="Minutes to wait before sending another alert for the same condition",
    )
...
class AlertCreate(AlertBase):
    ...
    initial_price: Optional[float] = Field(
        None, description="Initial price to set for the first alert check"
    )
...
class AlertUpdate(BaseModel):
    ...
    last_triggered_price: Optional[float] = Field(
        None, description="The price at which the alert was last triggered"
    )
...
class AlertInDB(AlertBase):
    ...
    user_name: Optional[str] = Field(
        None, description="Name of the user receiving alerts"
    )
    ...
    threshold_percent: float = Field(
        ..., description="Percentage change threshold for the alert"
    )
    last_triggered: Optional[datetime] = Field(
        None, description="When the alert was last triggered"
    )
    last_triggered_price: Optional[float] = Field(
        None, description="Price when the alert was last triggered"
    )
    status: AlertStatus = Field(
        AlertStatus.ACTIVE, description="Current status of the alert"
    )
