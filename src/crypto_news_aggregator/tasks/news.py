...
def fetch_news(self, source_name: Optional[str] = None, days: int = 1) -> Dict[str, Any]:
    """
    ...
    async def _fetch() -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            if source_name:
                logger.info(f"Collecting articles from source: {source_name} for the last {days} days")
                ...
                logger.info(f"Collecting articles from all sources for the last {days} days")
    ...
    try:
        ...
        if retries < self.max_retries:
            logger.warning(f"Retrying task {task_id} (attempt {retries + 1}/{self.max_retries})")
...
def process_article(self, article_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        retries = self.request.retries
        if retries < self.max_retries:
            logger.warning(f"Retrying article processing (attempt {retries + 1}/{self.max_retries})")
