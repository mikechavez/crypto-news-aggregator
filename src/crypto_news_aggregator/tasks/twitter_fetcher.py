...
class TwitterFetcher:
    ...
    async def start(self):
        ...
        if not self.user_ids:
            logger.error(
                "Could not resolve user IDs on startup. TwitterFetcher will not start."
            )
            return
        ...
        while True:
            try:
                ...
                logger.info(
                    f"Next in rotation: user {user_id_to_fetch} at index {self.current_user_index}"
                )
                ...
                self.current_user_index = (self.current_user_index + 1) % len(
                    self.user_ids
                )
