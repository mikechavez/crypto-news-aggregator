...
class UnsubscribeRequest(BaseModel):
    ...
    email_type: Optional[str] = (
        None  # Optional: unsubscribe from specific type of emails
    )
...
async def track_email_open(
    ...
    existing_open = next(
        (
            e
            for e in tracking.get("events", [])
            if e.get("event_type") == EmailEventType.OPENED
        ),
        None,
    )
    ...
    if not existing_open:
        ...
        await db.email_tracking.update_one(
            {"message_id": message_id}, {"$push": {"events": event}}, upsert=False
        )
...
async def unsubscribe_email(
    ...
    if unsubscribe_request and unsubscribe_request.email_type:
        update_data[f"subscription_preferences.{unsubscribe_request.email_type}"] = (
            False
        )
...
async def get_email_tracking(
    ...
    if (
        str(tracking["user_id"]) != str(current_user.id)
...
async def get_user_email_tracking(
    ...
    cursor = (
        db.email_tracking.find({"user_id": ObjectId(user_id)})
        .sort("sent_at", -1)
        .skip(skip)
        .limit(limit)
    )
