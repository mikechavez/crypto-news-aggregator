...
async def register_user(
    ...
    try:
        ...
        verification_url = (
            ...
            + f"api/v1/users/verify-email?token={user.email_verification_token}"
        )
...
async def reset_password(
    token: str, new_password: str, user_service: UserService = Depends(UserService)
) -> Dict[str, str]:
    """
...
async def resend_verification(
    ...
    # Generate verification URL
    verification_url = (
        str(request.base_url) + f"api/v1/users/verify-email?token={verification_token}"
    )
...
async def unsubscribe(
    ...
    success = await user_service.unsubscribe_user(
        unsubscribe_req.token, unsubscribe_req.email_type
    )
...
async def get_user(
    user_id: str, user_service: UserService = Depends(UserService)
) -> Any:
    """
