...
def has_non_serializable(obj: Any, path: str = "") -> tuple[bool, str]:
    ...
    if callable(obj):
        return (
            True,
            f"Callable object of type {type(obj).__name__} found at path: {path or 'root'}",
        )
    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            ...
            if not isinstance(k, (str, int, float, bool)) and k is not None:
                return (
                    True,
                    f"Non-serializable key of type {type(k).__name__} found at path: {path}",
                )
    ...
    try:
        ...
        return (
            True,
            f"Non-serializable object of type {type(obj).__name__} found at path: {path or 'root'}",
        )
...
def make_serializable(obj: Any) -> Any:
    ...
    if obj is None or isinstance(obj, (str, int, float, bool)):
        ...
        return {
            slot: make_serializable(getattr(obj, slot, None)) for slot in obj.__slots__
        }
    else:
        try:
...
async def get_task_status(
    ...
    response = TaskResponse(
        task_id=task_id, status="PENDING", result=None, error=None  # Default status
    )
    ...
    try:
        ...
        logger.info(
            f"[ENDPOINT] AsyncResult details: id={getattr(task_result, 'id', 'N/A')}, status={getattr(task_result, 'status', 'N/A')}"
        )
        ...
        try:
            ...
            logger.error(
                f"[ENDPOINT] Error checking if task is ready: {str(e)}", exc_info=True
            )
        ...
        logger.error(
            f"[ENDPOINT] Connection error getting task status: {error_msg}",
            exc_info=True,
        )
        response.status = "FAILURE"
    ...
    if is_ready:
        try:
            ...
            error_msg = str(e)
            logger.error(
                f"[DEBUG] Error getting task result: {error_msg}", exc_info=True
            )
            response.error = error_msg
...
@router.post(
    "/news/fetch",
    response_class=SerializableResponse,
