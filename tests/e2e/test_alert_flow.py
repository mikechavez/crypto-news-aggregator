...
async def test_user(client) -> UserInDB:
    ...
    user_data = UserCreate(
        email="test.e2e@example.com", password="password123", username="testuser"
    )
...
@pytest.mark.asyncio
@patch(
    "crypto_news_aggregator.services.price_service.price_service.get_bitcoin_price",
    new_callable=AsyncMock,
)
@patch(
    "crypto_news_aggregator.services.news_correlator.news_correlator.get_relevant_news",
    new_callable=AsyncMock,
)
@patch(
    "crypto_news_aggregator.services.email_service.email_service.send_price_alert",
    new_callable=AsyncMock,
)
async def test_btc_price_alert_flow(
    ...
    mock_get_relevant_news.return_value = [
        {
            "title": "BTC Dips Below 60k Amid Market Jitters",
    ...
    from crypto_news_aggregator.services.alert_notification_service import (
        alert_notification_service,
    )

    app.get("/test-trigger-alerts")(alert_notification_service.check_and_send_alerts)
    ...
    assert (
        call_args["news_articles"][0]["title"]
        == "BTC Dips Below 60k Amid Market Jitters"
    )
