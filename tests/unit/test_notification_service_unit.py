...
async def test_process_price_alert(
    notification_service, mock_mongo_manager, mock_email_service
):
...
async def test_check_alert_condition(notification_service):
    ...
    assert (
        notification_service._check_alert_condition(alert_above, 50000.0, 0) is False
    )  # Boundary
    ...
    assert (
        notification_service._check_alert_condition(alert_above, 0.01, 0) is False
    )  # Boundary
    ...
    assert (
        notification_service._check_alert_condition(alert_below, 50000.0, 0) is False
    )  # Boundary
    ...
    assert (
        notification_service._check_alert_condition(alert_below, 0.01, 0) is False
    )  # Boundary
    ...
    assert (
        notification_service._check_alert_condition(alert_pct_up, 0, 5.0) is True
    )  # Boundary (inclusive)
    ...
    assert (
        notification_service._check_alert_condition(alert_pct_up, 0, 0.1) is True
    )  # Boundary (inclusive)
    ...
    assert (
        notification_service._check_alert_condition(alert_pct_up, 0, 0.0) is True
    )  # Boundary (inclusive)
    ...
    assert (
        notification_service._check_alert_condition(alert_pct_down, 0, -5.0) is True
    )  # Boundary (inclusive)
    ...
    assert (
        notification_service._check_alert_condition(alert_pct_down, 0, -0.101) is True
    )
    assert (
        notification_service._check_alert_condition(alert_pct_down, 0, -0.1) is True
    )  # Boundary (inclusive)
    assert (
        notification_service._check_alert_condition(alert_pct_down, 0, -0.099) is False
    )
    ...
    assert (
        notification_service._check_alert_condition(alert_pct_down, 0, 0.0) is True
    )  # Boundary (inclusive)
...
@pytest.mark.asyncio
async def test_send_alert_notification_no_user(
    notification_service, mock_mongo_manager
):
