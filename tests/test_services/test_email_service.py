...
from src.crypto_news_aggregator.services.email_service import (
    EmailService,
    email_service,
)
...
TEST_HTML_CONTENT = (
    "<html><body><h1>Test</h1><a href='https://example.com'>Link</a></body></html>"
)
...
def mock_db():
    with patch(
        "src.crypto_news_aggregator.services.email_service.get_mongodb"
    ) as mock_get_db:
        mock_db = AsyncMock()
...
class TestEmailService:
    ...
    async def test_send_price_alert(self, mock_db):
        ...
        with patch(
            "src.crypto_news_aggregator.services.email_service.template_renderer"
        ) as mock_renderer:
            mock_renderer.render_template.return_value = TEST_HTML_CONTENT

            # Mock the send_email method
            with patch.object(
                self.service, "send_email", new_callable=AsyncMock
            ) as mock_send_email:
