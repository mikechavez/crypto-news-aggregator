...
class TestArticleServiceIntegration:
    ...
    async def test_get_top_articles_for_symbols_success(self, article_service):
        ...
        symbols = ['Bitcoin', 'BTC']
        result = await article_service.get_top_articles_for_symbols(symbols, hours=24, limit=5)
    ...
    async def test_get_top_articles_for_symbols_empty_result(self, article_service):
        ...
        result = await article_service.get_top_articles_for_symbols(symbols, hours=1, limit=5)
    ...
    async def test_get_top_articles_for_symbols_invalid_input(self, article_service):
        ...
        result = await article_service.get_top_articles_for_symbols([], hours=24, limit=5)
        assert isinstance(result, list)
        assert len(result) == 0

        # Test with None symbols
        result = await article_service.get_top_articles_for_symbols(None, hours=24, limit=5)
        assert isinstance(result, list)
        assert len(result) == 0

    async def test_get_top_articles_for_symbols_different_timeframes(self, article_service):
        """Test article retrieval with different timeframes."""
        symbols = ['Bitcoin']

        # Test with 1 hour
        result_1h = await article_service.get_top_articles_for_symbols(symbols, hours=1, limit=5)
        assert isinstance(result_1h, list)

        # Test with 24 hours
        result_24h = await article_service.get_top_articles_for_symbols(symbols, hours=24, limit=5)
        assert isinstance(result_24h, list)

        # Test with 7 days
        result_7d = await article_service.get_top_articles_for_symbols(symbols, hours=168, limit=5)
    ...
    async def test_get_top_articles_for_symbols_multiple_symbols(self, article_service):
        ...
        result = await article_service.get_top_articles_for_symbols(symbols, hours=24, limit=10)
    ...
    async def test_article_structure_validation(self, article_service):
        ...
        if result:  # If we have data
            for article in result:
                # Required fields
                required_fields = ['title', 'source', 'url', 'published_at', 'relevance_score', 'keywords']
...
class TestDatabaseConnectionIntegration:
    ...
    async def test_database_connection(self, article_service):
        ...
        try:
            ...
            result = await article_service.get_top_articles_for_symbols(['Bitcoin'], hours=1, limit=1)
            assert isinstance(result, list)
...
class TestArticleQueryIntegration:
    ...
    async def test_query_performance(self, article_service):
        ...
        result = await article_service.get_top_articles_for_symbols(['Bitcoin'], hours=24, limit=10)
        end_time = time.time()
