...
def mock_db():
    ...
    with patch(
        "src.crypto_news_aggregator.api.v1.endpoints.emails.get_mongodb"
    ) as mock_get_db:
        mock_db = AsyncMock()
...
class TestEmailTrackingEndpoints:
    ...
    async def test_track_email_click(self, client: TestClient, mock_db):
        ...
        mock_db.email_tracking.find_one.return_value = {
            ...
            "metadata": {"links": {TEST_LINK_HASH: TEST_ORIGINAL_URL}},
        }
    ...
    async def test_get_tracking_info(self, client: TestClient, mock_db, auth_headers):
        ...
        test_tracking_data = {
            ...
            "metadata": {"template_name": "test_template", "links": {}},
        }
        mock_db.email_tracking.find_one.return_value = test_tracking_data

        # Make the request
        response = client.get(
            f"/api/v1/emails/tracking/{TEST_MESSAGE_ID}", headers=auth_headers
        )
    ...
    async def test_get_user_tracking_info(
        self, client: TestClient, mock_db, auth_headers
    ):
    ...
    async def test_unauthorized_access_to_tracking_info(
        self, client: TestClient, mock_db
    ):
    ...
    async def test_unauthorized_access_to_user_tracking_info(
        self, client: TestClient, mock_db
    ):
        ...
        # Make the request
        response = client.get(
            f"/api/v1/emails/tracking/user/{TEST_USER_ID}", headers=headers
        )

        # Check the response
        assert response.status_code == 403

    async def test_admin_access_to_user_tracking_info(
        self, client: TestClient, mock_db
    ):
        """Test that admins can access any user's tracking info."""
        # Create an admin token
        admin_token = create_access_token(
            data={"sub": "admin_user_id", "is_admin": True}
        )
