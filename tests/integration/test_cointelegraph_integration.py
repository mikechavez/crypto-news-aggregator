...
async def test_fetch_articles_rate_limit(
    cointelegraph_source: CoinTelegraphSource, httpx_mock: HTTPXMock
):
    ...
    httpx_mock.add_response(json={"data": [], "pagination": {"total_items": 0}})
...
@pytest.mark.asyncio
async def test_format_article(
    cointelegraph_source: CoinTelegraphSource, sample_article: dict
):
