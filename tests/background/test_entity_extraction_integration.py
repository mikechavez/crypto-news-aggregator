...
@pytest.mark.asyncio
async def test_batch_processing_10_articles_success(
    mock_llm_client_success, sample_10_articles
):
    """Test successful batch processing of 10 articles."""
    result = await _process_entity_extraction_batch(
        sample_10_articles, mock_llm_client_success
    )
    ...
    assert (
        abs(usage["total_cost"] - 0.0024) < 0.0001
    )  # 10 * 0.00024 (with floating point tolerance)
...
@pytest.mark.asyncio
async def test_batch_processing_with_normalization(
    mock_llm_client_success, sample_10_articles
):
...
async def test_partial_failure_with_retry(
    mock_llm_client_partial_failure, sample_10_articles
):
    """Test partial failure scenario with individual retries."""
    result = await _process_entity_extraction_batch(
        sample_10_articles, mock_llm_client_partial_failure, retry_individual=True
    )
...
@pytest.mark.asyncio
async def test_partial_failure_no_retry(
    mock_llm_client_partial_failure, sample_10_articles
):
    """Test partial failure without retry returns empty results."""
    result = await _process_entity_extraction_batch(
        sample_10_articles, mock_llm_client_partial_failure, retry_individual=False
    )
...
@pytest.mark.asyncio
async def test_cost_tracking_from_api_response(
    mock_llm_client_success, sample_10_articles
):
