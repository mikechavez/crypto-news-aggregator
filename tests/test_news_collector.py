...
async def news_collector(mock_newsapi, mock_session) -> AsyncIterator[NewsCollector]:
    ...
    with patch(
        "crypto_news_aggregator.core.news_collector.get_sessionmaker"
    ) as mock_session_maker:
        # Configure the session maker to return our mock session
        mock_session_maker.return_value = AsyncContextManagerMock(
...
def mock_newsapi():
    ...
    with patch("crypto_news_aggregator.core.news_collector.NewsApiClient") as mock:
        ...
        api_client.get_sources.return_value = {"status": "ok", "sources": [test_source]}
...
@pytest.mark.asyncio
async def test_collect_from_source_api_error(
    news_collector, mock_newsapi, mock_session
):
...
async def test_collect_all_sources(news_collector, mock_newsapi, mock_session):
    ...
    assert (
        mock_newsapi.get_everything.call_count == 1
    ), "Expected get_everything to be called once"
...
async def test_retry_with_backoff(news_collector, mock_newsapi, mock_session):
    ...
    with patch(
        "crypto_news_aggregator.core.news_collector.asyncio.sleep"
    ) as mock_sleep:
        result = await news_collector.collect_from_source(source_id)

    # Verify the API was called 3 times (2 failures + 1 success)
    assert (
        mock_newsapi.get_everything.call_count == 3
    ), "Expected 3 API calls (2 failures + 1 success)"

    # Verify sleep was called with exponential backoff
    # Each retry includes a sleep, so we should have at least 2 sleeps for 2 retries
    assert (
        mock_sleep.await_count >= 2
    ), f"Expected at least 2 sleeps, got {mock_sleep.await_count}"
