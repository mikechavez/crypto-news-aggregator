...
TEST_SOURCE = Source(
    id="test-source-1", name="Test Source", url="https://example.com", type="news"
)
...
def mock_async_result_class():
    ...
    with patch(
        "src.crypto_news_aggregator.api.v1.tasks.CeleryAsyncResult"
    ) as mock_class:
        yield mock_class
...
def mock_celery_tasks():
    ...
    with (
        patch("crypto_news_aggregator.api.fetch_news", mock_fetch_news),
    ...
    try:
        ...
        with TestClient(test_app) as test_client:
            ...
            test_client.headers.update(
                {"X-API-Key": "testapikey123", "Content-Type": "application/json"}
            )
        ...
        ), f"Expected task_id '{task_id}', got '{data['task_id']}'"
        assert (
            data["status"] == "SUCCESS"
        ...
        ), f"Expected result {expected_result}, got {data.get('result')}"
...
def test_get_task_status_pending(client, monkeypatch, user_access_token, capsys):
    ...
    class MockAsyncResult:
        def __init__(self, task_id, **kwargs):
            print(
                f"[DEBUG] Creating Pending MockAsyncResult with task_id={task_id}, kwargs={kwargs}"
            )
            self.id = task_id
        ...
        def ready(self):
            print(
                f"[DEBUG] Pending MockAsyncResult.ready() called, returning {self._ready}"
            )
            return self._ready
    ...
    try:
        ...
        ), f"Expected result to be None for pending task, but got {data.get('result')}"
    finally:
...
def test_get_task_status_failed(
    mock_async_result_class, client, user_access_token, capsys
):
    ...
    assert (
        data["status"] == "FAILURE"
    ), f"Expected status 'FAILURE', got '{data['status']}'"

    # The endpoint might return either 'error' or include the error in 'result'
    if "error" in data:
        assert (
            error_message in data["error"]
        ), f"Expected error message to contain '{error_message}', but got '{data.get('error')}'"
    elif "result" in data and "error" in data["result"]:
        assert (
            error_message in data["result"]["error"]
        ), f"Expected error message in result to contain '{error_message}', but got '{data.get('result', {}).get('error')}'"


@pytest.mark.stable
def test_get_task_status_revoked(
    mock_async_result_class, client, user_access_token, capsys
):
    ...
    assert (
        data["status"] == "REVOKED"
    ), f"Expected status 'REVOKED', got '{data['status']}'"
    # The API includes result: None for all task statuses
    assert (
        data.get("result") is None
    ), f"Expected result to be None for revoked task, but got {data.get('result')}"


@pytest.mark.stable
def test_get_task_status_retry(
    mock_async_result_class, client, user_access_token, capsys
):
    ...
    assert (
        data.get("result") == retry_message
    ), f"Expected result '{retry_message}', got '{data.get('result')}'"


@pytest.mark.stable
def test_get_task_status_with_large_result(
    mock_async_result_class, client, user_access_token, capsys
):
    ...
    assert isinstance(
        result["data"], list
    ), f"Expected 'data' to be a list, got {type(result['data'])}"
    assert (
        len(result["data"]) == 1000
    ), f"Expected 1000 items in data, got {len(result['data'])}"

    # Verify the first item in the data list
    first_item = result["data"][0]
    assert (
        first_item == "x" * 1000
    ), f"Expected first item to be 'x' * 1000, got {first_item}"
...
async def test_get_task_status_with_exception(
    mock_celery_async_result, client, user_access_token
):
    ...
    type(mock_result).status = PropertyMock(
        side_effect=Exception("Failed to get status")
    )
    ...
    assert (
        "Failed to get status" in data["error"]
...
async def test_get_task_status_with_serialization_error(
    ...
    assert "error" in data, "Expected 'error' in response"
    assert (
        "not JSON serializable" in data["error"]
...
def test_get_task_status_with_custom_status(user_access_token):
    ...
    try:
        ...
        ), "Expected custom result in response"
...
async def test_trigger_news_fetch(
    mock_fetch_news, client, user_access_token, test_user
):
...
async def test_trigger_sentiment_analysis(client, user_access_token, test_user):
    ...
    with patch(
        "src.crypto_news_aggregator.api.v1.tasks.analyze_sentiment", mock_task
    ) as mock_patch:
...
async def test_trigger_trends_update(client, user_access_token, test_user):
    ...
    with patch(
        "src.crypto_news_aggregator.api.v1.tasks.update_trends", mock_task
    ) as mock_patch:
        print("\n[DEBUG] Mock task created and patched:")
        print(f"- mock_task: {mock_task}")
        print(f"- mock_result: {mock_result}")
...
def test_get_task_status_with_connection_error():
    ...
    try:
        ...
        assert "error" in data, "Expected 'error' field in response"
        assert (
            error_message.lower() in data["error"].lower()
...
def test_get_article_sentiment(client, mock_db_session, test_user):
    ...
    response = client.get(f"/api/v1/articles/{article_id}/sentiment", headers=headers)
...
@pytest.mark.stable
@pytest.mark.asyncio
async def test_trigger_sentiment_analysis_invalid_article(
    client, user_access_token, test_user
):
    ...
    with patch(
        ...
        assert len(args) == 1, f"Expected 1 positional argument, got {len(args)}"
        assert (
            args[0] == non_existent_article_id
