...
async def test_handle_price_movement_with_news(price_monitor):
    ...
    with patch('src.crypto_news_aggregator.tasks.price_monitor.notification_service', mock_notification), \
         patch('src.crypto_news_aggregator.tasks.price_monitor.NewsCorrelator', return_value=mock_correlator):
        ...
        mock_correlator.get_relevant_news.assert_called_once_with(
            price_change_percent=5.2,  # Should be absolute value
            max_articles=3
        )
        ...
        assert kwargs['context_articles'][0]['title'] == 'Bitcoin price surges after positive news'
