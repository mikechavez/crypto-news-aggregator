...
async def test_process_price_alert(notification_service, db_session, sample_alert):
    ...
    with (
        patch(
            "src.crypto_news_aggregator.services.notification_service.get_active_alerts",
            new_callable=AsyncMock,
        ) as mock_get_active_alerts,
        patch(
            "src.crypto_news_aggregator.services.notification_service.update_alert_last_triggered",
            new_callable=AsyncMock,
        ) as mock_update_alert,
        patch.object(
            notification_service, "_send_alert_notification", new_callable=AsyncMock
...
async def test_send_alert_notification_no_user(notification_service, sample_alert):
    ...
    mock_logger.warning.assert_called_with(
        f"Alert {sample_alert.id} has no associated user or email"
    )
