...
class TestPriceService:
    ...
    def test_trend_momentum_commentary(self, price_service):
        ...
        assert trend in ["strong bullish momentum", "strong bearish momentum", "a potential bullish reversal", "a potential bearish reversal", "a consolidation phase", "mixed signals"]
    ...
    async def test_should_trigger_alert(self, price_service):
        ...
        should_alert, change = await price_service.should_trigger_alert(51000.0, 50000.0, 2.0)
        assert should_alert is True
        assert change == 2.0

        should_alert, change = await price_service.should_trigger_alert(50100.0, 50000.0, 2.0)
    ...
    async def test_fetch_related_news_success(self, price_service):
        ...
        with patch('src.crypto_news_aggregator.services.price_service.article_service') as mock_article_service:
            mock_article_service.get_top_articles_for_symbols = AsyncMock(return_value=[
                {
    ...
    async def test_fetch_related_news_error(self, price_service):
        ...
        with patch('src.crypto_news_aggregator.services.price_service.article_service') as mock_article_service:
            mock_article_service.get_top_articles_for_symbols = AsyncMock(side_effect=Exception("DB Error"))
    ...
    async def test_generate_market_analysis_commentary_basic(self, price_service):
        ...
        with patch.object(price_service, 'get_markets_data') as mock_get_markets, \
