...
def mock_collection():
    ...
    with patch('src.crypto_news_aggregator.services.article_service.mongo_manager.get_async_collection') as mock_get_collection:
...
class TestArticleService:
    ...
    async def test_create_article_duplicate(self, mock_collection):
        ...
        with patch.object(service, '_update_duplicate_metadata', new_callable=AsyncMock) as mock_update:
    ...
    async def test_search_articles(self, mock_collection):
        ...
        assert pipeline[0]["$match"]["$and"][0]["$text"] == {"$caseSensitive": False, "$search": "bitcoin price"}
