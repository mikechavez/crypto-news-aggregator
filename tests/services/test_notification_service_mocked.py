...
def mock_mongo_manager():
    with patch('src.crypto_news_aggregator.services.notification_service.mongo_manager') as mock_mongo:
...
def notification_service(mock_mongo_manager):
    ...
    service._get_user = AsyncMock(return_value={
        'email': 'test@example.com',
        'name': 'Test User'
    })
    ...
    with patch('src.crypto_news_aggregator.services.notification_service.send_price_alert') as mock_send_alert:
...
async def test_process_price_alert():
    ...
    # Create test alerts
    test_alerts = [
        SAMPLE_ALERT.model_copy(update={"id": str(ObjectId()), "condition": AlertCondition.ABOVE, "threshold": 50000.0}),
        SAMPLE_ALERT.model_copy(update={"id": str(ObjectId()), "condition": AlertCondition.BELOW, "threshold": 60000.0}),
    ]
    ...
    with patch('src.crypto_news_aggregator.services.notification_service.mongo_manager', MockMongoManager()):
        # Call the method
        crypto_id = "bitcoin"
        crypto_name = "Bitcoin"
        crypto_symbol = "BTC"
...
async def test_send_alert_notification(notification_service):
    ...
    with patch('src.crypto_news_aggregator.services.notification_service.send_price_alert') as mock_send_alert, \
         patch('src.crypto_news_aggregator.services.notification_service.settings') as mock_settings:
        ...
        # Verify the result
        assert result is True, "Should return True when notification is sent successfully"
...
async def test_check_alert_condition():
    ...
    alert = SAMPLE_ALERT.model_copy(update={
        "condition": AlertCondition.ABOVE,
        "threshold": 50000.0
    })
...
async def test_get_recent_news():
    ...
    test_articles = [
        {"_id": "1", "title": f"{crypto_name.upper()} News 1", "url": "http://example.com/1"},
        {"_id": "2", "title": f"{crypto_name.upper()} News 2", "url": "http://example.com/2"}
    ]
    ...
    with (
        patch('src.crypto_news_aggregator.services.notification_service.mongo_manager', mock_mongo_manager),
        patch('src.crypto_news_aggregator.services.notification_service.datetime') as mock_dt
    ):
...
async def test_send_alert_notification_no_user(notification_service):
    ...
    with patch('src.crypto_news_aggregator.services.notification_service.logger') as mock_logger:
        # Call the method
        result = await service._send_alert_notification(
            alert=alert,
            crypto_name="Bitcoin",
