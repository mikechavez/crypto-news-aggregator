...
class InMemoryDB:
    def __init__(self):
        self.data = {"alerts": [], "users": [], "articles": []}

    async def find(
        self, collection: str, query: Dict, limit: Optional[int] = None
    ) -> List[Dict]:
...
class NotificationService:
    ...
    async def process_price_alert(
        ...
        alerts = await self.db.find(
            "alerts", {"crypto_id": crypto_id, "is_active": True}
        )
    ...
    def _check_alert_condition(
        self, alert: Dict, current_price: float, price_change_24h: float
    ) -> bool:
    ...
    async def _send_alert_notification(
        ...
        try:
            ...
            print(
                f"Subject: {crypto_name} Price Alert - {current_price:.2f} {crypto_symbol}"
            )
...
def setup_test_data(db):
    ...
    db.data["users"].append(
        {"_id": user_id, "email": "test@example.com", "name": "Test User"}
    )
